...
After Effects용 16bit 그레이스케일 출력
...
class DepthScannerApp:
    ...
    def build_ui(self):
        ...
        ttk.Checkbutton(opt_frame, text="16bit PNG 시퀀스 (AE용)", variable=self.seq_var).pack(side="left", padx=(15, 0))
    ...
    def process_image(self):
        ...
        out_path = str(inp.with_name(f"{inp.stem}_depth.png"))
        depth_inv = 1.0 - np.clip(depth_np, 0, 1)
    ...
    def process_video(self):
        ...
        inp = Path(self.input_path)
        self.update_status(f"영상: {w}x{h}, {fps:.1f}fps, {total}프레임")
        ...
        for i in range(total):
            ...
            if use_seq:
                gray_16 = (depth_inv * 65535).astype(np.uint16)
                if gray_16.shape[:2] != (h, w):
                    gray_16 = cv2.resize(gray_16, (w, h), interpolation=cv2.INTER_LINEAR)
                cv2.imwrite(os.path.join(out_dir, f"depth_{i:06d}.png"), gray_16)
            else:
