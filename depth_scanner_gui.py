...
class DepthScannerApp:
    ...
    def setup_styles(self):
        ...
        style.configure("Dim.TLabel",   foreground=self.colors["dim"],
                        font=("Segoe UI", 9))
        style.configure("Status.TLabel", foreground=self.colors["dim"],
                        font=("Consolas", 9))
    ...
    def build_ui(self):
        ...
        self.drop_frame = tk.Frame(
            inp_frame, bg=c["surface"],
            highlightbackground=c["border"], highlightthickness=2, cursor="hand2")
        self.drop_frame.pack(fill="x", ipady=25)

        self.drop_label = tk.Label(
            self.drop_frame,
            text="이미지 또는 영상을 여기에 드래그하거나 클릭하여 선택",
            bg=c["surface"], fg=c["dim"], font=("Segoe UI", 11))
        self.drop_label.pack(expand=True)

        self.drop_sub = tk.Label(
            self.drop_frame,
            text="jpg / png / tiff / mp4 / mov / avi — 최대 8K+ 해상도 지원",
            bg=c["surface"], fg=c["border"], font=("Consolas", 8))
        self.drop_sub.pack()
        ...
        ttk.Combobox(opt_frame, textvariable=self.steps_var,
                     values=["1", "2", "4", "10"], width=4,
                     state="readonly").pack(side="left", padx=(5, 15))

        ttk.Label(opt_frame, text="Ensemble:", style="Dim.TLabel").pack(side="left")
        self.ensemble_var = tk.StringVar(value="1")
        ttk.Combobox(opt_frame, textvariable=self.ensemble_var,
                     values=["1", "3", "5", "10"], width=4,
                     state="readonly").pack(side="left", padx=(5, 15))

        ttk.Label(opt_frame, text="Blend:", style="Dim.TLabel").pack(side="left")
        self.blend_var = tk.StringVar(value="0.3")
        ttk.Combobox(opt_frame, textvariable=self.blend_var,
                     values=["0", "0.2", "0.3", "0.5", "0.7"], width=4,
                     state="readonly").pack(side="left", padx=(5, 15))

        # 출력 형식 — 이미지/시퀀스 포맷 (exr/png16/png8)
        ttk.Label(opt_frame, text="포맷:", style="Dim.TLabel").pack(side="left", padx=(15, 0))
        self.format_var = tk.StringVar(value="exr")
        self.fmt_cb = ttk.Combobox(opt_frame, textvariable=self.format_var,
                                   values=["exr", "png16", "png8"], width=6,
                                   state="readonly")
        self.fmt_cb.pack(side="left", padx=(5, 0))

        # 영상 출력 모드 (영상일 때만 의미 있음)
        ttk.Label(opt_frame, text="영상출력:", style="Dim.TLabel").pack(side="left", padx=(15, 0))
        self.video_mode_var = tk.StringVar(value="sequence")
        ttk.Combobox(opt_frame, textvariable=self.video_mode_var,
                     values=["sequence", "mp4"], width=9,
                     state="readonly").pack(side="left", padx=(5, 0))

        ttk.Label(opt_frame, text="(영상에만 적용)", style="Dim.TLabel").pack(side="left", padx=(4, 0))

        # ── 버튼 영역 ──
        btn_frame = ttk.Frame(self.root)
        btn_frame.pack(fill="x", padx=20, pady=8)

        self.run_btn = ttk.Button(btn_frame, text="▶ Scan",
                                  style="Accent.TButton", command=self.start_scan)
        self.run_btn.pack(side="left")

        self.cancel_btn = ttk.Button(btn_frame, text="■ 취소",
                                     command=self.cancel_scan, state="disabled")
        self.cancel_btn.pack(side="left", padx=(10, 0))

        self.open_btn = ttk.Button(btn_frame, text="📁 출력 폴더 열기",
        ...
        left = tk.Frame(preview_frame, bg=c["surface"],
        ...
        right = tk.Frame(preview_frame, bg=c["surface"],
                         highlightbackground=c["border"], highlightthickness=1)
    ...
    def browse_file(self):
        ftypes = [
            ("지원 파일", "*.png *.jpg *.jpeg *.bmp *.tiff *.tif *.webp "
    ...
    def process_video(self):
        ...
        fmt        = self.format_var.get()          # exr / png16 / png8
        video_mode = self.video_mode_var.get()      # sequence / mp4
        use_seq    = (video_mode == "sequence")

        inp = Path(self.input_path)
        self.update_status(f"영상: {w}x{h}, {fps:.1f}fps, {total}프레임, 모드: {video_mode}")
        ...
        if use_seq:
            ...
            out_writer = cv2.VideoWriter(out_path, fourcc, fps, (w, h), isColor=False)
        ...
        try:
            for i in range(total):
                ...
                if use_seq:
                    if depth_inv.shape[:2] != (h, w):
                        depth_inv = cv2.resize(depth_inv.astype(np.float32),
                                               (w, h), interpolation=cv2.INTER_LINEAR)
                    self.save_depth(depth_inv, out_dir, f"depth_{i:06d}", fmt)
                else:
                    gray_8 = (depth_inv * 255).astype(np.uint8)
                    if gray_8.shape[:2] != (h, w):
                        gray_8 = cv2.resize(gray_8, (w, h), interpolation=cv2.INTER_LINEAR)
                    out_writer.write(gray_8)
            ...
            cap.release()
            # cancel 여부와 관계없이 항상 VideoWriter를 닫아 파일을 보호
            if out_writer is not None:
                out_writer.release()

        if not self.cancel_flag:
            if use_seq:
                self.update_status(f"완료: {out_dir}/ ({i+1}장 시퀀스)")
            else:
                self.update_status(f"완료: {out_path}")
            self.root.after(0, lambda: self.open_btn.config(state="normal"))
