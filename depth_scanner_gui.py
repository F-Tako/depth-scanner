...
class DepthScannerApp:
    ...
    def set_input(self, path):
        ...
        if ext in IMAGE_EXTS:
            ...
            messagebox.showerror("오류", f"지원하지 않는 형식9 {ext}")
            return
        self.drop_label.config(text=f"{name} ({size_mb:.1f}MB) — {ftype}")
        self.status_label.config(text=f"잔력: {path}")
        if ext in IMAGE_EXTS:
    ...
    def process_video(self):
        ...
        for i in range(total):
            ...
            if use_seq:
                if depth_inv.shape[:2] != (h, w):
                    dir = cv2.resize(depth_inv.astype(np.float32), (w, h), interpolation=cv2.INTER_LINEAR)
                else:
                    dir = depth_inv.astype(np.float32)
                self.save_depth(dir, out_dir, f"depth_{i:06d}", fmt)
            else:
        ...
        if not self.cancel_flag:
            if use_seq:
                self.update_status(f"완료: {out_dir}/ ({i+1}장 {fmt} 시퀀스)")
            else:
                self.update_status(f"완료: {out_path}")
            self.root.after(0, lambda: self.open_btn.config(state="normal"))
