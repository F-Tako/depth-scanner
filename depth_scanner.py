...
def process_image(
    ...
    if side_by_side:
        ...
        cv2.imwrite(output_path, gray_16)
...
def process_video_to_sequence(
    ...
    """영상 -> 16bit PNG 시퀀스 (AE에서 최고 품질로 import 가능)"""
    ...
    total_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
    fps = cap.get(cv2.CAP_PROP_FPS)
    print(f"[*] 영상 -> 16bit PNG 시퀀스: {output_dir}/")
    ...
    for i in tqdm(range(total_frames), desc="Depth 시퀀스 추출 중"):
        ...
        gray_16 = depth_to_gray(depth_np)
        out_path = os.path.join(output_dir, f"depth_{i:06d}.png")
        cv2.imwrite(out_path, gray_16)
        frame_count = i + 1

    cap.release()
    print(f"[+] 시퀀스 저장 완료: {output_dir}/ ({frame_count}장)")
    print(f"[i] AE: File > Import > File 에서 첫 번째 이미지 선택 > 'PNG Sequence' 체크")
...
예시:
  ...
  python depth_scanner.py video.mp4 --blend 0.5              temporal blending 강도
    ...
    parser.add_argument("--sequence", action="store_true",
                        help="영상을 16bit PNG 시퀀스로 출력 (AE용)")
    ...
    if is_image(args.input):
        out = args.output or str(inp.with_name(f"{inp.stem}_depth.png"))
        process_image(pipe, args.input, out, args.steps, args.ensemble, args.side_by_side)

    elif is_video(args.input):
        if args.sequence:
            out_dir = args.output or str(inp.with_name(f"{inp.stem}_depth_seq"))
            process_video_to_sequence(
                pipe, args.input, out_dir, args.steps, args.ensemble, args.blend
            )
        else:
